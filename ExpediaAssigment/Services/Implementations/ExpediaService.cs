...
using AutoMapper;
using Hotels.Models.ExpediaModels;
using Hotels.Models.SystemModels;
using Newtonsoft.Json;

namespace Hotels.Services
{
    public class ExpediaService : IHotelService
    {
        ...
        private string GetQueryString(SearchFilters filters)
        {
            ...
            {
                queryBuilder.AppendFormat("&minTripStartDate=:{0}", filters.MinTripStartDate.Value.ToString(ConstantData.DateTimeFormat));
            }

            if (filters.MaxTripStartDate.HasValue)
            {
                queryBuilder.AppendFormat("&maxTripStartDate=:{0}", filters.MaxTripStartDate.Value.ToString(ConstantData.DateTimeFormat));
            }
